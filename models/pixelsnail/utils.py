...
def get_causal_mask(canvas_size: int) -> tf.constant:
    """
    ...
    :param canvas_size: Height times width (the total number of spatial positions in the attention map);
                        e.g. for a 32x32 image HW=1024

    :return: the mask as tensor
    """

    causal_mask = np.zeros(shape=(canvas_size, canvas_size), dtype=np.float32)
